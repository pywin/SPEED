...
def translabel(mask, numcls):
    h, w = mask.shape
    newMask = np.zeros((numcls, h, w))
    for ichannel in range(numcls):
        newMask[ichannel] = np.where(mask==ichannel, 1, 0)
    return newMask
...
class BaseDataSet(Dataset):
    ...
    def _augmentation(self, image, label):
        ...
        if self.scale:
            h, w = int(h_org*self.scale), int(h_org*self.scale)
            image = cv2.resize(image, (w, h), interpolation=cv2.INTER_LINEAR)
